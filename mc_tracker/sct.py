...
class SingleCameraTracker:
    ...
    def process(self, frame, detections, mask=None):
        ...
        if self.reid_model:
            # si pende le detection dal frame
            #   => si salva le box di pixel e le usa come "features" per la comparazione con le detection successive
            reid_features = self._get_embeddings(frame, detections, mask)
        assignment = self._continue_tracks(detections, reid_features)   # cerca di assegnare gli id esistenti
        self._create_new_tracks(detections, reid_features, assignment)  #
        self._clear_old_tracks()  # rimuove le tracks o troppo vecchie, o troppo brevi
    ...
    def _continue_tracks(self, detections, features):
        ...
        for i, det1 in enumerate(detections):
            for j, det2 in enumerate(detections):
                if i != j and self._ios(det1, det2) > self.detection_occlusion_thresh: # seleziona le box che si sovrappongono
